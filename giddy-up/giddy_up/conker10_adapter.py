...
def _load_conker_runtime():
    for candidate in (REPO_ROOT.parent / "conker",):
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "ConkerTenAdapter requires an installed or sibling Conker runtime "
            "(for example ../conker)."
        ) from exc
    return mx, ConkerTenConfig, ConkerTenModel, ConkerThreeConfig, build_packed_tables, build_parameter_golf_dataset, scale_config
...
class ConkerTenAdapter:
    def __init__(self, summary_path: str, checkpoint_path: str, data_root: str | None = None):
        ...
        config = scale_config(
            ConkerTenConfig(
                ...
                structure_proxy_agreement_mass=bool(model_cfg.get("structure_proxy_agreement_mass", False)),
    ...
    def score_chunk(self, tokens: np.ndarray, sample_positions: np.ndarray | None = None) -> dict[str, Any]:
        ...
        if seq.size > 1:
            x = self._mx.array(seq[:-1][None, :], dtype=self._mx.int32)
