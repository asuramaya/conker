...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Conker-4b base variant: {args.variant}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-4b bridge on official parameter-golf token shards.")
    ...
    parser.set_defaults(gate_only_mode=False)
    ...
    parser.set_defaults(exact1_opens_mask=True)
    ...
    parser.set_defaults(special2_opens_mask=False)
    parser.add_argument("--special2-opens-mask", action="store_true", dest="special2_opens_mask")
    parser.add_argument("--no-special2-opens-mask", action="store_false", dest="special2_opens_mask")
    parser.set_defaults(number2_opens_mask=False)
    parser.add_argument("--number2-opens-mask", action="store_true", dest="number2_opens_mask")
    parser.add_argument("--no-number2-opens-mask", action="store_false", dest="number2_opens_mask")
    parser.set_defaults(urlpath2_opens_mask=False)
    parser.add_argument("--urlpath2-opens-mask", action="store_true", dest="urlpath2_opens_mask")
    parser.add_argument("--no-urlpath2-opens-mask", action="store_false", dest="urlpath2_opens_mask")
    parser.set_defaults(markup2_opens_mask=False)
    parser.add_argument("--markup2-opens-mask", action="store_true", dest="markup2_opens_mask")
    parser.add_argument("--no-markup2-opens-mask", action="store_false", dest="markup2_opens_mask")
    parser.set_defaults(attr2_opens_mask=False)
    parser.add_argument("--attr2-opens-mask", action="store_true", dest="attr2_opens_mask")
    parser.add_argument("--no-attr2-opens-mask", action="store_false", dest="attr2_opens_mask")
    parser.set_defaults(entity2_opens_mask=False)
    parser.add_argument("--entity2-opens-mask", action="store_true", dest="entity2_opens_mask")
    parser.add_argument("--no-entity2-opens-mask", action="store_false", dest="entity2_opens_mask")
    parser.set_defaults(stack2_opens_mask=False)
    parser.add_argument("--stack2-opens-mask", action="store_true", dest="stack2_opens_mask")
    parser.add_argument("--no-stack2-opens-mask", action="store_false", dest="stack2_opens_mask")
    parser.set_defaults(wordclass2_opens_mask=False)
    parser.add_argument("--wordclass2-opens-mask", action="store_true", dest="wordclass2_opens_mask")
    parser.add_argument("--no-wordclass2-opens-mask", action="store_false", dest="wordclass2_opens_mask")
    parser.set_defaults(delim2_opens_mask=True)
    ...
    parser.set_defaults(delimsub2_opens_mask=False)
    parser.add_argument("--delimsub2-opens-mask", action="store_true", dest="delimsub2_opens_mask")
    parser.add_argument("--no-delimsub2-opens-mask", action="store_false", dest="delimsub2_opens_mask")
    parser.add_argument(
        "--variant",
    ...
    config = scale_config(
        ConkerFourBConfig(
            ...
            special2_opens_mask=args.special2_opens_mask,
            number2_opens_mask=args.number2_opens_mask,
            urlpath2_opens_mask=args.urlpath2_opens_mask,
            markup2_opens_mask=args.markup2_opens_mask,
            attr2_opens_mask=args.attr2_opens_mask,
            entity2_opens_mask=args.entity2_opens_mask,
            stack2_opens_mask=args.stack2_opens_mask,
            wordclass2_opens_mask=args.wordclass2_opens_mask,
            delim2_opens_mask=args.delim2_opens_mask,
            delimsub2_opens_mask=args.delimsub2_opens_mask,
        ),
        args.scale,
    )
    model = ConkerFourBModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-4b golf bridge\n")
    print(
    ...
    print(
        ...
        f"freeze_base={config.freeze_base} "
        f"exact_context_span={config.exact_context_span} "
        f"residual_sources="
        f"{','.join(name for name, enabled in [('exact1', config.enable_exact1), ('exact2', config.enable_exact2), ('exact3', config.enable_exact3), ('special2', config.enable_special2), ('number2', config.enable_number2), ('urlpath2', config.enable_urlpath2), ('markup2', config.enable_markup2), ('attr2', config.enable_attr2), ('entity2', config.enable_entity2), ('stack2', config.enable_stack2), ('wordclass2', config.enable_wordclass2), ('delim2', config.enable_delim2), ('delimsub2', config.enable_delimsub2), ('recency', config.enable_recency)] if enabled)} "
        f"params={count_trainable_params(model):,}"
    )

    metrics = train_model(model, dataset, runtime.train, args.seed, "conker4b_golf_bridge")
    ...
    result = {
        "title": "conker-4b golf bridge cell",
        ...
        "model": {
            ...
            "tokenizer_vocab_path": config.tokenizer_vocab_path,
            "recency_half_life": config.recency_half_life,
            "exact_context_span": config.exact_context_span,
            "residual_cap": config.residual_cap,
            "base_feature_scale": config.base_feature_scale,
            "dynamic_support_gates": config.dynamic_support_gates,
            "dynamic_gate_span": config.dynamic_gate_span,
            "gate_only_mode": config.gate_only_mode,
            "support_gate_mode": config.support_gate_mode,
            "support_gate_topk": config.support_gate_topk,
            "support_gate_temperature": config.support_gate_temperature,
            "support_overlap_penalty": config.support_overlap_penalty,
            "exact1_opens_mask": config.exact1_opens_mask,
            "special2_opens_mask": config.special2_opens_mask,
            "number2_opens_mask": config.number2_opens_mask,
            "urlpath2_opens_mask": config.urlpath2_opens_mask,
            "markup2_opens_mask": config.markup2_opens_mask,
            "attr2_opens_mask": config.attr2_opens_mask,
            "entity2_opens_mask": config.entity2_opens_mask,
            "stack2_opens_mask": config.stack2_opens_mask,
            "wordclass2_opens_mask": config.wordclass2_opens_mask,
            "delim2_opens_mask": config.delim2_opens_mask,
            "delimsub2_opens_mask": config.delimsub2_opens_mask,
