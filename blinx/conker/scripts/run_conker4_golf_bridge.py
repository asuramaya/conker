...
from conker.src.conker4 import ConkerFourConfig, ConkerFourModel, scale_config
...
def neural_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "base":
        ...
        raise ValueError(f"Unknown Conker-4 neural variant: {args.variant}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-4 bridge on official parameter-golf token shards.")
    ...
    parser.add_argument("--count-alpha", type=float, default=0.05)
    parser.add_argument("--mixer-mode", choices=["support", "mlp"], default="support")
    parser.add_argument("--support-scale", type=float, default=0.5)
    parser.add_argument("--freeze-neural", action="store_true", default=True)
    parser.add_argument("--train-neural", action="store_false", dest="freeze_neural")
    parser.add_argument("--neural-bias-init", type=float, default=2.0)
    parser.add_argument("--aux-bias-init", type=float, default=-1.0)
    ...
    parser.add_argument("--mixer-hidden", type=int, default=32)
    parser.add_argument(
        "--variant",
    ...
    neural_cfg = neural_config_for_variant(args, runtime.train.seq_len)
    config = scale_config(
        ConkerFourConfig(
            neural_config=neural_cfg,
            enable_neural=not args.disable_neural,
            freeze_neural=args.freeze_neural,
            ...
            neural_bias_init=args.neural_bias_init,
            aux_bias_init=args.aux_bias_init,
            mixer_hidden=(args.mixer_hidden,),
        ),
        args.scale,
    )
    model = ConkerFourModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-4 golf bridge\n")
    print(
    ...
    print(
        f"  variant={args.variant} scale={args.scale:.3f} linear_modes={config.neural_config.linear_modes} "
        f"local_window={config.neural_config.local_window} "
        f"half_life_max={config.neural_config.linear_half_life_max:.1f} "
        f"osc_frac={config.neural_config.oscillatory_frac:.2f} "
        f"static_bank_gate={config.neural_config.static_bank_gate} "
        f"experts={','.join(model.expert_names)} params={count_trainable_params(model):,}"
    )

    metrics = train_model(model, dataset, runtime.train, args.seed, "conker4_golf_bridge")
    ...
    result = {
        "title": "conker-4 golf bridge cell",
        ...
        "model": {
            ...
            "seed": args.seed,
            "linear_modes": config.neural_config.linear_modes,
            "local_window": config.neural_config.local_window,
            "linear_half_life_max": config.neural_config.linear_half_life_max,
            "oscillatory_frac": config.neural_config.oscillatory_frac,
            "oscillatory_period_min": config.neural_config.oscillatory_period_min,
            "oscillatory_period_max": config.neural_config.oscillatory_period_max,
            "static_bank_gate": config.neural_config.static_bank_gate,
            "recency_half_life": config.recency_half_life,
            "count_alpha": config.count_alpha,
            "mixer_mode": config.mixer_mode,
            "support_scale": config.support_scale,
            "freeze_neural": config.freeze_neural,
            "neural_bias_init": config.neural_bias_init,
            "aux_bias_init": config.aux_bias_init,
