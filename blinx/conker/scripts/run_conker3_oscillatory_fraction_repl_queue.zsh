...
STAMP="$(date +%Y-%m-%d)"
LOG_PATH="$OUT_DIR/conker3_oscillatory_fraction_repl_queue_${STAMP}.log"
...
echo "starting conker-3 oscillatory fraction replication queue" | tee "$LOG_PATH"
...
echo "conker-3 oscillatory fraction replication queue complete" | tee -a "$LOG_PATH"
