...
LOG="$CONKER/out/conker2_ablation_queue_2026-03-25.log"

mkdir -p "$CONKER/out"
echo "starting conker-2 ablation queue" | tee "$LOG"
...
for variant in base untied_base linear_only correction_only equal_logit untied_equal_logit no_bias probability_mix; do
  for seed in 42 43 44; do
    run_cell "$variant" "$seed" "$CONKER/out/conker2_${variant}_golf_bridge_seed${seed}_2026-03-25.json"
  done
done

echo "conker-2 ablation queue complete" | tee -a "$LOG"
