...
LOG="$CONKER/out/conker2_linear_scale_queue_2026-03-25.log"

mkdir -p "$CONKER/out"
echo "starting conker-2 linear scale queue" | tee "$LOG"
...
run_cell() {
  ...
  local json="$CONKER/out/conker2_linear_only_scale_${scale_tag}_golf_bridge_seed${seed}_2026-03-25.json"
  ...
  echo "run conker2 linear_only scale=$scale seed=$seed" | tee -a "$LOG"
  cd "$ROOT"
  "$PYTHON" "$CONKER/scripts/run_conker2_golf_bridge.py" \
    ...
    --profile pilot \
    --json "$json" | tee -a "$LOG"
  echo "done conker2 linear_only scale=$scale seed=$seed" | tee -a "$LOG"
}

for scale in 1.75 2.0; do
  for seed in 42 43 44; do
    run_cell "$scale" "$seed"
  done
done

echo "conker-2 linear scale queue complete" | tee -a "$LOG"
