...
from conker.src.conker9 import ConkerNineConfig, ConkerNineModel, scale_config
...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Conker-9 base variant: {args.variant}")
...
def parse_lag_lookbacks(raw: str) -> tuple[int, ...]:
    ...
    if not values:
        raise ValueError("Conker-9 requires at least one lag lookback.")
    return tuple(values)


def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-9 bridge: causal controller over fixed legal lag buckets.")
    ...
    model = ConkerNineModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-9 golf bridge\n")
    print(
    ...
    metrics = train_model(model, dataset, runtime.train, args.seed, "conker9_golf_bridge")
    ...
    result = {
        "title": "conker-9 golf bridge cell",
