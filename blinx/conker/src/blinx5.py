...
@dataclass
class Blinx5Compressed:
    original_length: int
    final_bytes: bytes
    rounds: List[Blinx5Round]
...
def _positions_from_mask(mask: List[bool]) -> Tuple[int, ...]:
    return tuple(index for index, removed in enumerate(mask) if removed)
...
def _encode_varint(value: int) -> bytes:
    if value <= 0:
        raise ValueError("BLX5 varint expects a positive integer")
    out = bytearray()
    current = value
    while current >= 0x80:
...
def _decode_varint(stream: BytesIO) -> int:
    ...
    while True:
        blob = stream.read(1)
        if len(blob) != 1:
            raise ValueError("Unexpected end of BLX5 varint payload")
        byte = blob[0]
        ...
        shift += 7
        if shift > 63:
            raise ValueError("BLX5 varint overflow")
    if value <= 0:
        raise ValueError("BLX5 varint decoded a non-positive delta")
    return value
...
def _decode_removed_positions(payload: bytes, length_before: int) -> Tuple[int, ...]:
    ...
    while stream.tell() < len(payload):
        ...
        if position < 0 or position >= length_before:
            raise ValueError("BLX5 decoded a removed position outside the round length")
...
def _round_variants(
    *,
    ...
    if mask_policy != MASK_POLICY_ADAPTIVE:
        raise ValueError(f"Unknown BLX5 mask policy: {mask_policy}")
    variants = [
        Blinx5Round(
            length_before=length_before,
            context_radius=context_radius,
        ...
        for dictionary_cap in phase.dictionary_caps:
            ...
            for pair_rule_budget in phase.pair_rule_budgets:
                for round_state in _round_variants(
                    ...
                    candidate_zlib = Blinx5Compressed(
...
def _read_exact(stream: BytesIO, size: int) -> bytes:
    ...
    if len(blob) != size:
        raise ValueError("Unexpected end of BLX5 payload")
    return blob


def deserialize(payload: bytes) -> Blinx5Compressed:
    stream = BytesIO(payload)
    if _read_exact(stream, 4) != MAGIC:
        raise ValueError("Not a BLX5 payload")
    version, original_length, round_count, pair_rule_budget, final_length = struct.unpack(
        "<BIIII",
        _read_exact(stream, 17),
    )
    if version != VERSION:
        raise ValueError(f"Unsupported BLX5 version: {version}")
    ...
    for _ in range(round_count):
        ...
        if mask_format == MASK_FORMAT_BITSET:
            ...
            raise ValueError(f"Unsupported BLX5 mask format: {mask_format}")
        rounds.append(
            Blinx5Round(
                length_before=length_before,
                context_radius=context_radius,
    ...
    if keys_read != total_key_count:
        raise ValueError("BLX5 key count mismatch")
...
def decompress(compressed: Blinx5Compressed) -> bytes:
    ...
    for round_state in reversed(compressed.rounds):
        ...
        if survivor_index != len(current):
            raise ValueError("BLX5 survivor length mismatch during rebuild")
        for index in removed_positions:
            radius = round_state.context_radius
            if index < radius or index >= len(rebuilt) - radius:
                raise ValueError("BLX5 attempted to remove boundary byte")
            ...
            if any(value is None for value in left) or any(value is None for value in right):
                raise ValueError("BLX5 encountered overlapping removals within a round")
            key = bytes(left + right)  # type: ignore[arg-type]
            if key not in (round_state.dictionary or {}):
                raise ValueError(f"BLX5 missing dictionary entry for context {key}")
    ...
    if len(current) != compressed.original_length:
        raise ValueError("BLX5 output length mismatch")
    return current
