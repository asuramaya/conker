...
@dataclass
class Blinx3Compressed:
    original_length: int
    final_bytes: bytes
    rounds: List[Blinx3Round]
    ...
    def serialize(self) -> bytes:
        out = BytesIO()
        out.write(MAGIC)
        out.write(struct.pack("<BIIII", VERSION, self.original_length, len(self.rounds), self.pair_rule_budget, len(self.final_bytes)))
...
def compress(
    ...
    *,
    ...
    for _ in range(max_rounds):
        best_step: Tuple[bytes, Blinx3Round] | None = None
        ...
        for context_radius in candidate_radii:
            ...
            candidate_zlib = Blinx3Compressed(
                original_length=len(data),
            ...
            if selection_mode == "profit":
                ...
                raise ValueError(f"Unknown BLX3 selection mode: {selection_mode}")
...
def _read_exact(stream: BytesIO, size: int) -> bytes:
    ...
    if len(blob) != size:
        raise ValueError("Unexpected end of BLX3 payload")
    return blob


def deserialize(payload: bytes) -> Blinx3Compressed:
    stream = BytesIO(payload)
    if _read_exact(stream, 4) != MAGIC:
        raise ValueError("Not a BLX3 payload")
    version, original_length, round_count, pair_rule_budget, final_length = struct.unpack(
        "<BIIII", _read_exact(stream, 17)
    )
    if version != VERSION:
        raise ValueError(f"Unsupported BLX3 version: {version}")
    ...
    if keys_read != total_key_count:
        raise ValueError("BLX3 key count mismatch")
...
def decompress(compressed: Blinx3Compressed) -> bytes:
    ...
    for round_state in reversed(compressed.rounds):
        ...
        if survivor_index != len(current):
            raise ValueError("BLX3 survivor length mismatch during rebuild")
        for index, removed in enumerate(removed_mask):
            if not removed:
                continue
            radius = round_state.context_radius
            if index < radius or index >= len(rebuilt) - radius:
                raise ValueError("BLX3 attempted to remove boundary byte")
            ...
            if any(value is None for value in left) or any(value is None for value in right):
                raise ValueError("BLX3 encountered overlapping removals within a round")
            key = bytes(left + right)  # type: ignore[arg-type]
            if key not in round_state.dictionary:
                raise ValueError(f"BLX3 missing dictionary entry for context {key}")
    ...
    if len(current) != compressed.original_length:
        raise ValueError("BLX3 output length mismatch")
    return current
