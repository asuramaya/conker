...
Operationally, the causal reset still comes from the inherited `Conker` line while:

- `conker-detect` owns the audits
- `conker-ledger` owns the validity bundle and public report layer

Separate from that causal line, there is now a noncausal lossless-compression branch:

- [BLINX1.md](./BLINX1.md)
- it does not compete on `bpb`
- it exists to test iterative removal / denoising as a standalone compression process
