...
`Conker` validity lessons from the audited tandem and future-teacher lines:
...
Current conservative benchmark ladder for the tandem branch:
...
That is the minimum ladder future `Conker-5` and `Conker-7` claims should satisfy.

For structural and side-channel audits outside the training runtime, use [`conker-detect`](https://github.com/asuramaya/conker-detect).
