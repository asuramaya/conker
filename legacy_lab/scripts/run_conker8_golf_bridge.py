...
from conker.src.conker8 import ConkerEightConfig, ConkerEightModel, scale_config
...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Conker-8 base variant: {args.variant}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-8 bridge: strict legal structural learning on Conker-4b.")
    ...
    parser.add_argument("--lag-profile-span", type=float, default=0.5)
    parser.add_argument("--support-mask-span", type=float, default=0.5)
    parser.add_argument("--disable-learn-lag-profile", action="store_true")
    parser.add_argument("--disable-learn-delimiter-mask", action="store_true")
    parser.add_argument("--disable-learn-number-mask", action="store_true")
    parser.add_argument("--disable-learn-special-mask", action="store_true")
    parser.add_argument("--disable-learn-urlpath-mask", action="store_true")
    parser.add_argument("--disable-learn-markup-mask", action="store_true")
    parser.add_argument("--disable-learn-attr-mask", action="store_true")
    parser.add_argument("--disable-learn-entity-mask", action="store_true")
    ...
    config = scale_config(
        ConkerEightConfig(
            ...
            learn_lag_profile=not args.disable_learn_lag_profile,
            lag_profile_span=args.lag_profile_span,
            learn_delimiter_mask=not args.disable_learn_delimiter_mask,
            learn_number_mask=not args.disable_learn_number_mask,
            learn_special_mask=not args.disable_learn_special_mask,
            learn_urlpath_mask=not args.disable_learn_urlpath_mask,
            learn_markup_mask=not args.disable_learn_markup_mask,
            learn_attr_mask=not args.disable_learn_attr_mask,
            learn_entity_mask=not args.disable_learn_entity_mask,
            support_mask_span=args.support_mask_span,
        ),
        args.scale,
    )
    model = ConkerEightModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-8 golf bridge\n")
    print(
    ...
    print(
        ...
        f"lag_profile={config.learn_lag_profile} lag_span={config.lag_profile_span:.2f} "
        f"support_mask_span={config.support_mask_span:.2f} "
        f"residual_sources="
    ...
    metrics = train_model(model, dataset, runtime.train, args.seed, "conker8_golf_bridge")
    ...
    result = {
        "title": "conker-8 golf bridge cell",
        ...
        "model": {
            ...
            "seed": args.seed,
            "lag_profile_span": config.lag_profile_span,
            "support_mask_span": config.support_mask_span,
            "learn_lag_profile": config.learn_lag_profile,
            "learn_delimiter_mask": config.learn_delimiter_mask,
            "learn_number_mask": config.learn_number_mask,
            "learn_special_mask": config.learn_special_mask,
            "learn_urlpath_mask": config.learn_urlpath_mask,
            "learn_markup_mask": config.learn_markup_mask,
            "learn_attr_mask": config.learn_attr_mask,
            "learn_entity_mask": config.learn_entity_mask,
