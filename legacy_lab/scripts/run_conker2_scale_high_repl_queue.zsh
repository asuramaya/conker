...
LOG="$CONKER/out/conker2_scale_high_repl_queue_2026-03-25.log"

mkdir -p "$CONKER/out"
echo "starting conker-2 high-scale replication queue" | tee "$LOG"
...
for scale in 2.5 3.0; do
  for seed in 43 44; do
    run_cell "$scale" "$seed"
  done
done

echo "conker-2 high-scale replication queue complete" | tee -a "$LOG"
