...
STAMP="$(date +%Y-%m-%d)"
LOG_PATH="$OUT_DIR/conker3_oscillatory_probe_queue_${STAMP}.log"
...
echo "starting conker-3 oscillatory probe queue" | tee "$LOG_PATH"

run_probe 0.25 42
run_probe 0.50 42

echo "conker-3 oscillatory probe queue complete" | tee -a "$LOG_PATH"
