...
LOG="$CONKER/out/conker2_golf_queue_2026-03-25.log"

mkdir -p "$CONKER/out"
echo "starting conker-2 golf queue" | tee "$LOG"
...
run_cell() {
  ...
  echo "run conker2 seed $seed" | tee -a "$LOG"
  cd "$ROOT"
  "$PYTHON" "$CONKER/scripts/run_conker2_golf_bridge.py" \
    ...
    --linear-modes 256 \
    --json "$json" | tee -a "$LOG"
  echo "done conker2 seed $seed" | tee -a "$LOG"
}

for seed in 42 43 44; do
  run_cell "$seed" "$CONKER/out/conker2_golf_bridge_seed${seed}_2026-03-25.json"
done

echo "conker-2 golf queue complete" | tee -a "$LOG"
