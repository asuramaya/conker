...
from conker.src.asura_tetra import AsuraTetraConfig, AsuraTetraModel, scale_config
...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Asura-Tetra base variant: {args.variant}")
...
def parse_lag_lookbacks(raw: str) -> tuple[int, ...]:
    ...
    if not values:
        raise ValueError("Asura-Tetra requires at least one lag lookback.")
    return tuple(values)


def main() -> None:
    parser = argparse.ArgumentParser(description="Asura-Tetra bridge: compact four-axis mask-program controller over legal lag buckets.")
    ...
    model = AsuraTetraModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  asura tetra golf bridge\n")
    print(
    ...
    metrics = train_model(model, dataset, runtime.train, args.seed, "asura_tetra_golf_bridge")
    ...
    result = {
        "title": "asura tetra golf bridge cell",
