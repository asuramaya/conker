...
The `Conker-5/7` ancestry is contaminated by a real training bug in `Conker-4b`.

Root cause:

- `Conker-4b` used tuple-valued `freeze(keys=...)` calls.
- MLX `freeze()` only honors `list[str]` here; tuples get stored as one composite `_no_grad` key.
- So these were trainable in fresh `Conker-4b` and therefore in tandem `Conker-5` and warm-started `Conker-7`:
  - `causal_mask`
  - `vocab_axis`
  - `token_class_ids`
  - `delimiter_mask`
...
- the old tandem / warm-start frontier was overwhelmingly powered by accidentally trainable structural buffers
- once those buffers actually freeze, the intended residual exact-expert branch falls to about `2.06–2.10 bpb`
- the pre-fix `Conker-7` result should now be treated as a contaminated branch built on top of that ancestor
- next honest work is strict rebuild, not incremental warm-starting from the old tandem line

## Conker-8

`Conker-8` is now the first explicit legal rebuild around the ashes of the freeze bug.
...
- keep the patched strict `Conker-4b` branch
- turn the accidental hidden structure into explicit trainables:
  - strict lower-triangular lag profile only
  - within-support token weights only

First pilot, seed `42`, `window4 / 10x / 256 / batch16 / 1000 / lr5e-4`, tandem, gate-only + dynamic support gates:
...
- the minimal legal structural rebuild does not recover the old contaminated gains
- neither the explicit lag profile nor the explicit support-mask weights matter on their own in this first form
- the next real branch should move structure into gate/control decisions rather than only into weighted count masks
...
- stronger lag learning remains inert
- stronger support-mask learning is the unstable part
- disabling dynamic support gates does not rescue it
- enabling recency makes the legal branch worse
- the next honest `Conker-8` move is explicit control/routing, not stronger weighted-mask amplitudes

## Conker-9

`Conker-9` is the first causal-controller rebuild after the dead `Conker-8` mask branch.
...
- patched strict `Conker-4b` base
- fixed legal lag buckets `(2, 4, 8, 16, 32, 64, 128, full)`
- controller over causal base features chooses a softmax mixture over those buckets
- blended bucket features feed the existing strict residual stack

First pilot, seed `42`, `window4 / 10x / 256 / batch16 / 1000 / lr5e-4`:
...
- the lag controller is real, not a no-op
- but legal horizon selection alone still does not beat the strict `~2.06` floor
- the next honest branch should target controller decisions over:
  - opener/candidate creation
...
- fixed packed training-memory priors from `1,000,000` train tokens
- exact unigram prior
- exact bigram counts
- hashed trigram counts with `2,048` buckets
- normalized posterior backoff
- causal controller mixing:
  - base
  - unigram
  - bigram
  - trigram

First pilot, seed `42`, `window4 / 10x / 256 / batch16 / 500 / lr5e-4`:
...
- mean controller source weights:
  ...
  - bigram `0.0019`
  - trigram `0.0011`
- controller parameter magnitudes:
  - hidden weight mean abs `0.2080`
  - output weight mean abs `0.1816`
- average bigram mass `4945.0`
- average trigram mass `643.6`

Read:

- this is not a no-op controller
- the packed memory tables are populated and available
- but the controller overwhelmingly routes back to the base
- this first memory-first branch is worse than the strict `~2.06` floor

Immediate next experiments:

- cache-only / memory-only baseline with the same packed tables
- fixed interpolation baselines over `[base, unigram, bigram, trigram]`
...
Same packed tables (`1,000,000` train tokens, `2,048` trigram buckets):
...
- fixed interpolation baseline:
  - mode `fixed_interp`
  - fixed weights `[0.20, 0.05, 0.25, 0.50]` over `[base, unigram, bigram, trigram]`
  - seed `42`, `500` steps
...
- fixed interpolation with heavy memory use is also much worse than the learned mixer
- so the first `Conker-10` failure is not merely a conservative controller
- the packed tables themselves are weak in their current construction
