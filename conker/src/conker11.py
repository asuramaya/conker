...
CONKER11_FEATURE_DIM = 11


@dataclass(frozen=True)
class ConkerElevenConfig(ConkerNineConfig):
...
class ConkerElevenModel(ConkerNineModel):
    """Strict causal residual routing with lag control, source ownership, and opener control."""

    def __init__(self, vocab_size: int, config: ConkerElevenConfig = ConkerElevenConfig()):
        super().__init__(vocab_size=vocab_size, config=config)
        source_count = len(ROUTED_SOURCE_NAMES)
        self.conker11_source_feature_weights = mx.zeros((source_count, CONKER11_FEATURE_DIM), dtype=mx.float32)
        self.conker11_source_mass_weights = mx.zeros((source_count,), dtype=mx.float32)
        self.conker11_source_hit_weights = mx.zeros((source_count,), dtype=mx.float32)
        self.conker11_source_bias = mx.zeros((source_count,), dtype=mx.float32)
        self.conker11_source_abstain_feature_weights = mx.zeros((CONKER11_FEATURE_DIM,), dtype=mx.float32)
        self.conker11_source_abstain_bias = mx.array(0.0, dtype=mx.float32)

        self.conker11_opener_feature_weights = mx.zeros((source_count, CONKER11_FEATURE_DIM), dtype=mx.float32)
        self.conker11_opener_mass_weights = mx.zeros((source_count,), dtype=mx.float32)
        self.conker11_opener_hit_weights = mx.zeros((source_count,), dtype=mx.float32)
        self.conker11_opener_bias = mx.zeros((source_count,), dtype=mx.float32)

        self.conker11_residual_feature_weights = mx.zeros((CONKER11_FEATURE_DIM,), dtype=mx.float32)
        self.conker11_residual_bias = mx.array(0.0, dtype=mx.float32)

    @property
    def routing_config(self) -> ConkerElevenConfig:
        return self.config

    def _conker11_global_lag_bucket_idx(self) -> int | None:
        for idx, lookback in enumerate(self.routing_config.lag_lookbacks):
            if lookback <= 0:
                return idx
        return None

    def _conker11_lag_mass_masks(self) -> tuple[mx.array, mx.array, mx.array]:
    ...
    def _conker11_lag_controller(self, base_features: mx.array) -> mx.array:
        logits = (
        ...
        global_idx = self._conker11_global_lag_bucket_idx()
    ...
    def _conker11_features(self, base_features: mx.array, lag_weights: mx.array) -> mx.array:
        short_mask, long_finite_mask, global_mask = self._conker11_lag_mass_masks()
    ...
    def _conker11_source_controller_logit(
        self,
        source_idx: int,
        features: mx.array,
        source: mx.array,
        ...
        return (
            mx.sum(features * self.conker11_source_feature_weights[source_idx][None, None, :], axis=-1)
            + self.conker11_source_mass_weights[source_idx] * mass
            + self.conker11_source_hit_weights[source_idx] * hit
            + self.conker11_source_bias[source_idx]
        ).astype(dtype)

    def _conker11_opener_controller_logit(
        self,
        source_idx: int,
        features: mx.array,
        source: mx.array,
        ...
        mass, hit = self._source_mass_and_hit(source, dtype)
        return (
            mx.sum(features * self.conker11_opener_feature_weights[source_idx][None, None, :], axis=-1)
            + self.conker11_opener_mass_weights[source_idx] * mass
            + self.conker11_opener_hit_weights[source_idx] * hit
            + self.conker11_opener_bias[source_idx]
        ).astype(dtype)

    def _conker11_source_router(
        self,
        features: mx.array,
        ...
        for name, source in active_items:
            ...
            logits.append(self._conker11_source_controller_logit(source_idx, features, source, dtype))
            names.append(name)
        abstain_logit = (
            mx.sum(features * self.conker11_source_abstain_feature_weights[None, None, :], axis=-1)
            + self.conker11_source_abstain_bias
        ).astype(dtype)
    ...
    def _conker11_opener_router(
        ...
        for name, source in sources.items():
            ...
            activation = mx.sigmoid(self._conker11_opener_controller_logit(source_idx, features, source, dtype) / temperature) * hit
            out[name] = activation[..., None]
        return out

    def _conker11_residual_scale(self, features: mx.array) -> mx.array:
        logit = (
            mx.sum(features * self.conker11_residual_feature_weights[None, None, :], axis=-1)
            + self.conker11_residual_bias
        )
    ...
    def _routing_state(
        ...
        lag_weights = self._conker11_lag_controller(base_features)
        sources = self._source_dict_from_tuple(self._count_features_with_controller(chars, lag_weights))
        route_features = self._conker11_features(base_features, lag_weights)
        source_probs = self._conker11_source_router(route_features, sources, base_logits.dtype)
        opener_probs = self._conker11_opener_router(route_features, sources, base_logits.dtype)
        residual_scale = self._conker11_residual_scale(route_features)
    ...
    def controller_snapshot(self, chars: mx.array) -> dict[str, object]:
        ...
        global_lag_idx = self._conker11_global_lag_bucket_idx()
...
def scale_config(config: ConkerElevenConfig, scale: float) -> ConkerElevenConfig:
    if scale == 1.0:
        return config
    return replace(
        config,
