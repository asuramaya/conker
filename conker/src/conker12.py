...
from conker.src.conker11 import CONKER11_FEATURE_DIM, ConkerElevenConfig, ConkerElevenModel, ROUTED_SOURCE_NAMES
from conker.src.conker3 import scale_config as scale_conker3_config


CONKER12_EFFECT_DIM = 2


@dataclass(frozen=True)
class ConkerTwelveConfig(ConkerElevenConfig):
    program_slots: int = 4
...
class ConkerTwelveModel(ConkerElevenModel):
    """Conker-11 plus a compact four-axis program tensor over lag, source, slot, and effect."""

    def __init__(self, vocab_size: int, config: ConkerTwelveConfig = ConkerTwelveConfig()):
        ...
        self.conker12_program_feature_weights = mx.zeros((slot_count, CONKER11_FEATURE_DIM), dtype=mx.float32)
        self.conker12_program_bias = mx.zeros((slot_count,), dtype=mx.float32)
        self.conker12_program_tensor = mx.zeros((lag_count, source_count, slot_count, CONKER12_EFFECT_DIM), dtype=mx.float32)

    @property
    def twelve_config(self) -> ConkerTwelveConfig:
        return self.config

    def _program_mix(self, features: mx.array) -> mx.array:
        logits = (
            mx.sum(features[..., None, :] * self.conker12_program_feature_weights[None, None, :, :], axis=-1)
            + self.conker12_program_bias[None, None, :]
        )
        temperature = max(self.twelve_config.program_temperature, 1e-4)
    ...
    def _program_effects(
        ...
        joint = (
            ...
            * self.conker12_program_tensor[None, None, :, :, :, :].astype(dtype)
        )
        effects = mx.sum(joint, axis=(2, 4))
        route_deltas = self.twelve_config.program_route_span * mx.tanh(effects[..., 0])
        opener_deltas = self.twelve_config.program_opener_span * mx.tanh(effects[..., 1])
        route_map = {name: route_deltas[..., idx] for idx, name in enumerate(ROUTED_SOURCE_NAMES)}
        opener_map = {name: opener_deltas[..., idx] for idx, name in enumerate(ROUTED_SOURCE_NAMES)}
    ...
    def _source_router_programmed(
        ...
        for name, source in active_items:
            ...
            base_logit = self._conker11_source_controller_logit(source_idx, features, source, dtype)
    ...
    def _opener_router_programmed(
        ...
        for name, source in sources.items():
            ...
            base_logit = self._conker11_opener_controller_logit(source_idx, features, source, dtype)
...
def scale_config(config: ConkerTwelveConfig, scale: float) -> ConkerTwelveConfig:
    if scale == 1.0:
        return config
    return replace(
        config,
