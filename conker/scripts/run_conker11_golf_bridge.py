...
from conker.src.conker11 import ConkerElevenConfig, ConkerElevenModel, scale_config
...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Conker-11 base variant: {args.variant}")
...
def parse_lag_lookbacks(raw: str) -> tuple[int, ...]:
    ...
    if not values:
        raise ValueError("Conker-11 requires at least one lag lookback.")
    return tuple(values)


def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-11 bridge: strict causal recursive router over fixed legal lag buckets.")
    ...
    model = ConkerElevenModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-11 golf bridge\n")
    print(
    ...
    metrics = train_model(model, dataset, runtime.train, args.seed, "conker11_golf_bridge")
    ...
    result = {
        "title": "conker-11 golf bridge cell",
