...
from conker.src.conker11 import ConkerElevenConfig
from conker.src.conker12 import ConkerTwelveConfig, ConkerTwelveModel, scale_config
...
def base_config_for_variant(args: argparse.Namespace, seq_len: int) -> ConkerThreeConfig:
    ...
    if args.variant == "window4":
        ...
        raise ValueError(f"Unknown Conker-12 base variant: {args.variant}")
...
def parse_lag_lookbacks(raw: str) -> tuple[int, ...]:
    ...
    if not values:
        raise ValueError("Conker-12 requires at least one lag lookback.")
    return tuple(values)


def main() -> None:
    parser = argparse.ArgumentParser(description="Conker-12 bridge: higher-order program tensor over the Conker-11 controller stack.")
    ...
    parser.add_argument("--program-route-span", type=float, default=1.0)
    parser.add_argument("--program-opener-span", type=float, default=1.0)
    ...
    config = scale_config(
        ConkerTwelveConfig(
            ...
            program_route_span=args.program_route_span,
            program_opener_span=args.program_opener_span,
        ),
        args.scale,
    )
    model = ConkerTwelveModel(vocab_size=dataset.vocab_size, config=config)

    print("\n  conker-12 golf bridge\n")
    print(
    ...
    print(
        ...
        f"program_slots={config.program_slots} program_temp={config.program_temperature:.2f} "
        f"route_span={config.program_route_span:.2f} opener_span={config.program_opener_span:.2f} "
        f"params={count_trainable_params(model):,}"
    )

    metrics = train_model(model, dataset, runtime.train, args.seed, "conker12_golf_bridge")
    ...
    result = {
        "title": "conker-12 golf bridge cell",
        ...
        "model": {
            ...
            "program_route_span": config.program_route_span,
            "program_opener_span": config.program_opener_span,
    ...
    if args.save_state:
        save_state_npz(Path(args.save_state), dict(nn.utils.tree_flatten(model.parameters())))
