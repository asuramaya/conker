...
LOG="$CONKER/out/conker2_cap_pilot_queue_2026-03-25.log"

mkdir -p "$CONKER/out"
echo "starting conker-2 cap-neighborhood pilot queue" | tee "$LOG"
...
for scale in 8.0 10.0 12.0; do
  run_cell "$scale" 42
done

echo "conker-2 cap-neighborhood pilot queue complete" | tee -a "$LOG"
